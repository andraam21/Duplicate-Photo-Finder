...
import skimage.color as skycol  # designed for image preprocessing
...
from typing import List  # provides runtime support for type hints
import numpy as np
import cv2  # designed to solve computer vision problems
import os  # provides functions for interacting with the operating system
import time
import collections  # implements specialized container datatypes providing
# alternatives to Python's general purpose built-in containers, dict , list , set , and tuple .
from pathlib import Path  # provides various classes representing file system paths
import argparse  # automatically generates help and usage messages and issues errors
import json  # provides a lot of methods to read the JSON file
import warnings
...
class dif:
    ...
    def __init__(self, directory_A, directory_B=None, recursive=True, similarity="normal", px_size=50, show_progress=True, show_output=False, delete=False, silent_del=False):
        ...
        similarity (str, int)...."normal" = searches for duplicates, recommended setting, MSE < 200 (Mean Squared Error)
        ...
        px_size (int)............resize images to px_size height x width (in pixels) before being compared
        ...
        delete (bool)............lower resolution duplicate images that were found are automatically deleted
        silent_del (bool)........True = skips the asking for user confirmation when deleting lower resolution duplicate images
        ...
        *** Comand Line Interface***
        ...
        if directory_B == None:
            ...
            img_matrices_A, folderfiles_A = dif._create_imgs_matrix(
                directory_A, px_size, recursive, show_progress)
            ref = dif._map_similarity(similarity)
            result, lower_quality, total = dif._search_one_dir(img_matrices_A, folderfiles_A,ref, show_output, show_progress)
        else:
            ...
            img_matrices_A, folderfiles_A = dif._create_imgs_matrix(
                directory_A, px_size, recursive, show_progress)
            img_matrices_B, folderfiles_B = dif._create_imgs_matrix(
        ...
        stats = dif._generate_stats(directory_A, directory_B,
                                    time.localtime(start_time), time.localtime(
    ...
    def _validate_parameters(self, show_output, show_progress, recursive, similarity, px_size, delete, silent_del):
    ...
    def _process_directory(self, directory):
        ...
        if not os.path.isdir(directory):
            raise FileNotFoundError(
                "Directory " + str(directory) + " does not exist")
        return directory

    # Function that validates paths to exclude erroneous duplicates finding
    def _path_validation(self, paths: List[Path]) -> None:
        if len(set(paths)) == 1:
    ...
    def _create_imgs_matrix(self, directory, px_size, recursive, show_progress):

        # create list of tuples with files found in directory, format: (path, filename)
        folder_files = [(directory, filename)
                        for filename in os.listdir(directory)]
        if recursive is True:
            subfolders = dif._find_subfolders(directory)
            if len(subfolders) >= 1:
                for folder in subfolders:
                    subfolder_files = [(folder, filename)
                ...
                if not os.path.isdir(path):
                    try:
                        ...
                        if isinstance(img, np.ndarray):
                            img = img[..., 0:3] # r, g, b = img[..., 0], img[..., 1], img[..., 2]
    ...
    def _search_one_dir(img_matrices_A, folderfiles_A, similarity, show_output=True, show_progress=False):
                ...
                if count_B > count_A and count_A != len(img_matrices_A):
                    ...
                    while rotations <= 3:
                        ...
                        if err < ref:
                                ...
                                for id in result.keys():
                                    for dupl in result[id]["duplicates"]:
                                        if os.path.samefile(dupl, str(Path(folderfiles_A[count_B][0]) / folderfiles_A[count_B][1])):
                                            result[id]["duplicates"] = result[id]["duplicates"] + [
                                                str(Path(folderfiles_A[count_A][0]) / folderfiles_A[count_A][1])]
                                            check = True
                                            break
                            if not check:
                            ...
                            try:
                                high, low = dif._check_img_quality(Path(
                                    folderfiles_A[count_A][0]) / folderfiles_A[count_A][1], Path(folderfiles_A[count_B][0]) / folderfiles_A[count_B][1])
            ...
            for count_B, imageMatrix_B in enumerate(img_matrices_B):
                ...
                while rotations <= 3:
                    ...
                    if err < ref:
                        ...
                        if img_id in result.keys():
                            result[img_id]["duplicates"] = result[img_id]["duplicates"] + \
                                [str(Path(folderfiles_B[count_B][0]) /
                                     folderfiles_B[count_B][1])]
                        else:
                            result[img_id] = {'filename': str(folderfiles_A[count_A][1]),
                                              'location': str(Path(folderfiles_A[count_A][0]) / folderfiles_A[count_A][1]),
                        ...
                        try:
                            high, low = dif._check_img_quality(Path(
                                folderfiles_A[count_A][0]) / folderfiles_A[count_A][1], Path(folderfiles_B[count_B][0]) / folderfiles_B[count_B][1])
                            lower_quality.append(str(low))
                        except:
                            pass
                        break
    ...
    def _show_progress(count, list, task='processing images'):
        if count+1 == len(list):
            print(
                f"DifPy {task}: [{count}/{len(list)}] [{count/len(list):.0%}]", end="\r")
            print(
                f"DifPy {task}: [{count+1}/{len(list)}] [{(count+1)/len(list):.0%}]")
        else:
            print(
                f"DifPy {task}: [{count}/{len(list)}] [{count/len(list):.0%}]", end="\r")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-A", "--directory_A", type=str,
                        help='Directory to search for images.', required=True)
    parser.add_argument("-B", "--directory_B", type=str,
                        help='(optional) Second directory to search for images.', required=False, nargs='?', default=None)
    parser.add_argument("-r", "--recursive", type=bool, help='(optional) Scan subfolders for duplicate images',
                        required=False, nargs='?', choices=[True, False], default=True)
    parser.add_argument("-Z", "--output_directory", type=str,
                        help='(optional) Output directory for the difPy result files. Default is working dir.', required=False, nargs='?', default=None)
    parser.add_argument("-s", "--similarity", type=type_str_int,
                        help='(optional) Similarity grade.', required=False, nargs='?', default='normal')
    parser.add_argument("-px", "--px_size", type=int,
                        help='(optional) Compression size of images in pixels.', required=False, nargs='?', default=50)
    ...
    parser.add_argument("-D", "--silent_del", type=bool, help='(optional) Supresses the user confirmation when deleting images.',
                        required=False, nargs='?', choices=[True, False], default=False)
